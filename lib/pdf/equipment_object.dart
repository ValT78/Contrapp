...
List<pw.Widget> buildEquipment(List<Equipment> equipments, pw.TextStyle style, pw.TextStyle styleBold) {
  ...
  for (var equipment in equipments) {
    ...
    rows.addAll(buildTableRows(equipment.machines, style));
  }

  int rowsPerPage = 30; // Adjust this value based on your page size and row height
  for (int i = 0; i < rows.length; i += rowsPerPage) {
    pages.add(
      pw.Table(
        ...
        children: rows.sublist(i, i + rowsPerPage > rows.length ? rows.length : i + rowsPerPage),
