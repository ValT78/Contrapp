...
class HomePage extends StatelessWidget {
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: <Widget>[
                ...
                const SuperTitle(
                    title: "Ouverts Récemment", color: Colors.grey),
                const Divider(),
                ...oldContractPaths.keys.toList().reversed.map((key) {
                  return Column(
                    children: [
                      InkWell(
                        onTap: () async {
                          bool asLoaded = await loadContractData(
                              oldContractPaths[key]?['path'], context);
                        ...
                        child: ListTile(
                          leading: const Icon(
                            ...
                            size: 30,
                          ), // Ajout de l'icône de fichier
                          title: Row(
                            children: [
                              Text(key,
                                  style: const TextStyle(
                                      fontSize:
                                          20)), // Ajout du nom du f ichier
                              const Spacer(),
                              Text(
                                  oldContractPaths[key]?['date'] != null
                                      ? DateFormat('yyyy-MM-dd HH:mm').format(
                                          DateTime.parse(
                                              oldContractPaths[key]!['date']!))
                                      : '',
                                  style: const TextStyle(
                                      fontSize:
                                          15)), // Ajout de la date de dernière modification
                            ],
                          ),
                        ),
                      ),
  ...
  Future<bool> loadContractData(
      String? contractPath, BuildContext? context) async {
    FilePickerResult? contract;
    String? contractJsonData;
    if (contractPath == null) {
      ...
      if (contract != null && contract.files.single.path != null) {
        ...
        contractJsonData = await file.readAsString();
      }
    } else {
      try {
    ...
    if (contractJsonData != null) {
      Map<String, dynamic> data = jsonDecode(contractJsonData);
      resetAppData();
